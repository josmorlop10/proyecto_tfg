...
;--------------------------------------------------------
	.area _CODE
;src/main.c:25: void init_gfx(void){
;	---------------------------------
; Function init_gfx
; ---------------------------------
_init_gfx::
;src/main.c:27: set_sprite_data(0, 4, duck);
	ld	de, #_duck
	push	de
	ld	hl, #0x400
	push	hl
...
;/home/josem/gbdk/include/gb/gb.h:1887: shadow_OAM[nb].tile=tile;
	...
	ld	(hl), #0x03
;src/main.c:34: set_sprite_data(4, 4, selector);
	ld	de, #_selector
	push	de
	ld	hl, #0x404
	push	hl
...
;/home/josem/gbdk/include/gb/gb.h:1887: shadow_OAM[nb].tile=tile;
	...
	ld	(hl), #0x07
;src/main.c:41: set_sprite_data(8,8,object_sprites);
...
;src/main.c:50: SHOW_SPRITES;
	...
	or	a, #0x02
	ldh	(_LCDC_REG + 0), a
;src/main.c:53: set_bkg_data(0, 104, map_tiles_alt);
	...
	call	_set_bkg_data
	add	sp, #4
;src/main.c:54: set_bkg_tiles(0,0,20,16,map1_alt);
	ld	de, #_map1_alt
	push	de
	ld	hl, #0x1014
	push	hl
...
;src/main.c:59: set_win_data(104,22, hud_tiles);
	...
	call	_set_win_data
	add	sp, #4
;src/main.c:60: set_win_tiles(0,0,20,3, hud_selector);
	ld	de, #_hud_selector
	push	de
	ld	hl, #0x314
	push	hl
	...
	call	_set_win_tiles
	add	sp, #6
;src/main.c:61: SHOW_WIN;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x20
	ldh	(_LCDC_REG + 0), a
;src/main.c:62: WX_REG = 7;      // SIEMPRE 7
	ld	a, #0x07
	ldh	(_WX_REG + 0), a
;src/main.c:63: WY_REG = 120;    // 144 - 24
	ld	a, #0x78
	ldh	(_WY_REG + 0), a
;src/main.c:64: }
	ret
...
; ---------------------------------
_main::
;src/main.c:68: init_gfx();
	call	_init_gfx
;src/main.c:69: pointer_init(&s);
	ld	de, #_s
	call	_pointer_init
;src/main.c:71: global_game_state = STATE_GAME_SETTING;
...
00101$:
;src/main.c:77: if(last_state != STATE_GAME_SETTING) {
	ld	a, (#_last_state)
	sub	a, #0x02
	jr	Z, 00103$
...
;src/main.c:83: last_state = STATE_GAME_SETTING;
	ld	hl, #_last_state
	ld	(hl), #0x02
00103$:
;src/main.c:85: update_pointer(&s);
	ld	de, #_s
	call	_update_pointer
;src/main.c:86: if(joypad() & J_START){
	call	_joypad
	rlca
	jr	NC, 00118$
;src/main.c:87: hide_pointer();
	call	_hide_pointer
;src/main.c:88: update_game_state(STATE_GAME_RUNNING);
	ld	a, #0x03
	call	_update_game_state
;src/main.c:90: break;
	jr	00118$
;src/main.c:92: case STATE_GAME_RUNNING:
00106$:
;src/main.c:93: if(last_state != STATE_GAME_RUNNING) {
	ld	a, (#_last_state)
	sub	a, #0x03
	jr	Z, 00108$
;src/main.c:94: character_init(&p);
	ld	de, #_p
	call	_character_init
;src/main.c:95: last_state = STATE_GAME_RUNNING;
	ld	hl, #_last_state
	ld	(hl), #0x03
00108$:
;src/main.c:97: update_character(&p);
	ld	de, #_p
	call	_update_character
;src/main.c:100: if(joypad() & J_A){
	call	_joypad
	bit	4, a
	jr	Z, 00118$
