...
;--------------------------------------------------------
	.area _CODE
;src/LevelLogic.c:46: void init_game_title(void){
;	---------------------------------
; Function init_game_title
; ---------------------------------
_init_game_title::
;src/LevelLogic.c:48: HIDE_WIN;
	ldh	a, (_LCDC_REG + 0)
	and	a, #0xdf
	ldh	(_LCDC_REG + 0), a
;src/LevelLogic.c:49: HIDE_SPRITES;
	ldh	a, (_LCDC_REG + 0)
	and	a, #0xfd
	ldh	(_LCDC_REG + 0), a
;src/LevelLogic.c:50: set_bkg_data(0,232,png_prueba_tiles);
	...
	call	_set_bkg_data
	add	sp, #4
;src/LevelLogic.c:51: set_bkg_tiles(0,0,20,18,png_prueba_map);
	...
	call	_set_bkg_tiles
	add	sp, #6
;src/LevelLogic.c:52: SHOW_BKG;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x01
	ldh	(_LCDC_REG + 0), a
;src/LevelLogic.c:53: }
	ret
;src/LevelLogic.c:55: void init_level(uint8_t level_number){
;	---------------------------------
; Function init_level
; ---------------------------------
_init_level::
	...
	dec	sp
	ld	c, a
;src/LevelLogic.c:57: global_keyset = 0;
;src/LevelLogic.c:58: global_hud_selected = 0;
	xor	a, a
	ld	(#_global_keyset), a
	ld	(#_global_hud_selected),a
;src/LevelLogic.c:59: if(level_number >= LEVEL_COUNT){
	ld	a, c
	sub	a, #0x03
	jr	C, 00115$
;src/LevelLogic.c:60: level_number = level_number % LEVEL_COUNT;
	ld	e, #0x03
	ld	a, c
	call	__moduchar
;src/LevelLogic.c:63: for(uint16_t i = 0; i<NUMBER_OF_TILES_IN_GRID; i++){
00115$:
	ld	de, #0x0000
00106$:
	ld	a, e
	...
	sbc	a, #0x01
	jr	NC, 00103$
;src/LevelLogic.c:64: global_colision_map[i] = EMPTY;
...
;src/LevelLogic.c:63: for(uint16_t i = 0; i<NUMBER_OF_TILES_IN_GRID; i++){
	...
	jr	00106$
00103$:
;src/LevelLogic.c:67: get_colision_from_map(global_levels_array[level_number], global_colision_map);
...
;src/LevelLogic.c:68: read_global_object_info_from_map(global_level_objects_array[level_number]);
...
;src/LevelLogic.c:69: read_global_block_info_from_map(global_level_blocks_array[level_number]);
...
;src/LevelLogic.c:71: for(uint8_t e = 0; e < NUMBER_OF_BLOCKS; e++){
	ld	c, #0x00
00109$:
	ld	a, c
	sub	a, #0x06
	jr	NC, 00104$
;src/LevelLogic.c:72: update_values_in_hud(RIGHT+e, global_blocks_available[e]);
...
;src/LevelLogic.c:71: for(uint8_t e = 0; e < NUMBER_OF_BLOCKS; e++){
	...
	jr	00109$
00104$:
;src/LevelLogic.c:75: get_init_point_from_map(global_colision_map);
...
;src/LevelLogic.c:77: }
	...
	inc	sp
	ret
;src/LevelLogic.c:79: void update_game_state(GameState new_value){
;	---------------------------------
; Function update_game_state
; ---------------------------------
_update_game_state::
	ld	(#_global_game_state),a
;src/LevelLogic.c:80: global_game_state = new_value;
;src/LevelLogic.c:81: }
	ret
;src/LevelLogic.c:84: void get_colision_from_map(const unsigned char in[], uint8_t out[]){
;	---------------------------------
...
_get_colision_from_map::
	...
	ld	(hl+), a
	ld	(hl), b
;src/LevelLogic.c:86: for(uint16_t i = 0; i<NUMBER_OF_TILES_IN_GRID; i++){
	ld	bc, #0x0000
00130$:
	ld	e, c
	ld	d, b
	...
	sbc	a, #0x01
	jp	NC, 00132$
;src/LevelLogic.c:87: if(in[i] == 51){
	ldhl	sp,	#3
	ld	a,	(hl+)
	ld	h, (hl)
	ld	l, a
	...
	ld	e, l
	ld	d, h
;src/LevelLogic.c:97: } else if(in[i] >= 21 && in[i] <= 29){
	ld	a, (de)
	ldhl	sp,	#0
;src/LevelLogic.c:88: out[i] = SOURCE_R;
	ld	(hl+), a
	ld	a,	(hl+)
	ld	h, (hl)
	ld	l, a
...
;src/LevelLogic.c:88: out[i] = SOURCE_R;
	...
	jr	00131$
00126$:
;src/LevelLogic.c:89: } else if(in[i] == 55){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x37
	jr	NZ, 00123$
;src/LevelLogic.c:90: out[i] = SOURCE_L;
	ld	a, #0x0e
	ld	(de), a
	jr	00131$
00123$:
;src/LevelLogic.c:91: } else if(in[i] == 59){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x3b
	jr	NZ, 00120$
;src/LevelLogic.c:92: out[i] = SOURCE_U;
	ld	a, #0x0f
	ld	(de), a
	jr	00131$
00120$:
;src/LevelLogic.c:93: } else if(in[i] == 63){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x3f
	jr	NZ, 00117$
;src/LevelLogic.c:94: out[i] = SOURCE_D;
	ld	a, #0x10
	ld	(de), a
	jr	00131$
00117$:
;src/LevelLogic.c:95: } else if(in[i] >= 64 && in[i] <= 67){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x40
	jr	C, 00113$
	...
	sub	a, (hl)
	jr	C, 00113$
;src/LevelLogic.c:96: out[i] = DESTINATION;
	ld	a, #0x04
	ld	(de), a
	jr	00131$
00113$:
;src/LevelLogic.c:97: } else if(in[i] >= 21 && in[i] <= 29){
	...
	sub	a, (hl)
	jr	C, 00109$
;src/LevelLogic.c:98: out[i] = FALL;
	ld	a, #0x0c
	ld	(de), a
	jr	00131$
00109$:
;src/LevelLogic.c:99: } else if(in[i] >= UMBRAL_COLISION_UP && in[i] <= UMBRAL_COLISION_DOWN){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x05
	jr	C, 00105$
	...
	sub	a, (hl)
	jr	C, 00105$
;src/LevelLogic.c:100: out[i] = SOLID;
	ld	a, #0x01
	ld	(de), a
	jr	00131$
00105$:
;src/LevelLogic.c:101: } else if(in[i] >= 32 && in[i] <= 39){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x20
	jr	C, 00131$
	...
	sub	a, (hl)
	jr	C, 00131$
;src/LevelLogic.c:102: out[i] = DOOR;
	ld	a, #0x0d
	ld	(de), a
00131$:
;src/LevelLogic.c:86: for(uint16_t i = 0; i<NUMBER_OF_TILES_IN_GRID; i++){
	...
	jp	00130$
00132$:
;src/LevelLogic.c:105: }
	add	sp, #5
	ret
;src/LevelLogic.c:107: void read_global_object_info_from_map(const unsigned char* objects_map){
;	---------------------------------
...
_read_global_object_info_from_map::
	...
	ld	a, e
	ld	(hl+), a
;src/LevelLogic.c:108: for(uint8_t e=0; e<NUMBER_OF_OBJECTS; e++){
	ld	a, d
	ld	(hl+), a
	ld	(hl), #0x00
00103$:
	...
	sub	a, #0x08
	jr	NC, 00105$
;src/LevelLogic.c:109: global_object_information[e*3] = objects_map[e*3];
	ld	c, (hl)
	ld	b, #0x00
	ld	l, c
	ld	h, b
	...
	ld	l, e
	ld	(hl), a
;src/LevelLogic.c:110: global_object_information[e*3+1] = objects_map[e*3+1];
	ldhl	sp,	#5
	ld	a, (hl)
	ld	e, a
	add	a, a
	...
	ld	l, e
	ld	(hl), a
;src/LevelLogic.c:111: global_object_information[e*3+2] = objects_map[e*3+2];
	ldhl	sp,	#0
	ld	a, (hl)
	inc	a
	inc	a
...
;src/LevelLogic.c:113: }
	add	sp, #6
	ret
;src/LevelLogic.c:115: void read_global_block_info_from_map(const unsigned char* blocks_map){
;	---------------------------------
...
_read_global_block_info_from_map::
	add	sp, #-1
	push	de
;src/LevelLogic.c:116: for(uint8_t e=0; e<NUMBER_OF_BLOCKS; e++){
	ldhl	sp,	#2
	ld	(hl), #0x00
00103$:
	ldhl	sp,	#2
	...
	sub	a, #0x06
	jr	NC, 00105$
;src/LevelLogic.c:117: global_blocks_available[e] = blocks_map[e];
...
;src/LevelLogic.c:119: }
	add	sp, #3
	ret
;src/LevelLogic.c:121: void change_colision_map_at(uint16_t tileindexBR, uint8_t new_value){
;	---------------------------------
...
;src/LevelLogic.c:123: global_colision_map[tileindexBR] = new_value;
	...
	ld	a, (hl)
	ld	(de), a
;src/LevelLogic.c:124: global_colision_map[tileindexBR-1] = new_value;
	ld	e, c
	ld	d, b
	dec	de
	ld	hl, #_global_colision_map
	...
	ld	a, (hl)
	ld	(de), a
;src/LevelLogic.c:125: global_colision_map[tileindexBR-20] = new_value;
	ld	a, c
	add	a, #0xec
	ld	e, a
	ld	a, b
	...
	ld	a, (hl)
	ld	(de), a
;src/LevelLogic.c:126: global_colision_map[tileindexBR-21] = new_value;
	ld	a, c
	add	a, #0xeb
	ld	c, a
	ld	a, b
...
;src/LevelLogic.c:128: }
	inc	sp
	ret
;src/LevelLogic.c:130: void change_colision_map_BR(uint16_t tileindexBR, uint8_t new_value){
;	---------------------------------
...
;src/LevelLogic.c:132: global_colision_map[tileindexBR] = new_value;
	...
	add	hl, de
	ld	(hl), c
;src/LevelLogic.c:134: }
	ret
;src/LevelLogic.c:136: uint8_t check_colision_of_sprites(uint8_t ax, uint8_t ay, uint8_t aw, uint8_t ah, uint8_t bx, uint8_t by, uint8_t bw, uint8_t bh){
;	---------------------------------
...
_check_colision_of_sprites::
	...
	ldhl	sp,	#5
	ld	(hl), e
;src/LevelLogic.c:137: uint8_t res = 0; //no colision of sprites
	ldhl	sp,	#0
	ld	(hl), #0x00
;src/LevelLogic.c:139: if(ax < bx + bw &&
	ldhl	sp,	#10
	ld	a, (hl)
	ldhl	sp,	#1
	ld	(hl+), a
...
;src/LevelLogic.c:142: ay + ah > by){
	...
	sbc	a, b
	jr	NC, 00102$
;src/LevelLogic.c:143: res = 1;
	dec	hl
	dec	hl
	ld	(hl), #0x01
00102$:
;src/LevelLogic.c:145: return res;
	ldhl	sp,	#0
	ld	a, (hl)
;src/LevelLogic.c:146: }
	...
	add	sp, #6
	jp	(hl)
;src/LevelLogic.c:149: void get_init_point_from_map(uint8_t colision_map[NUMBER_OF_TILES_IN_GRID]){
;	---------------------------------
