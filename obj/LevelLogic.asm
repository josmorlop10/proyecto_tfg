...
;src/LevelLogic.c:56: out[i] = DESTINATION;
	...
	jr	00114$
00105$:
;src/LevelLogic.c:57: } else if(in[i] >= UMBRAL_COLISION_UP && in[i] <= UMBRAL_COLISION_DOWN){
	ldhl	sp,	#0
	ld	a, (hl)
	sub	a, #0x03
	jr	C, 00114$
	...
	sub	a, (hl)
	jr	C, 00114$
;src/LevelLogic.c:58: out[i] = SOLID;
	ld	a, #0x01
	ld	(de), a
00114$:
;src/LevelLogic.c:52: for(uint16_t i = 0; i<360; i++){
...
;src/LevelLogic.c:61: }
	add	sp, #5
	ret
;src/LevelLogic.c:63: void read_global_object_info_from_map(unsigned char* objects_map){
;	---------------------------------
...
_read_global_object_info_from_map::
	...
	ld	a, e
	ld	(hl+), a
;src/LevelLogic.c:64: for(uint8_t e=0; e<NUMBER_OF_OBJECTS; e++){
	ld	a, d
	ld	(hl+), a
	ld	(hl), #0x00
00103$:
	...
	sub	a, #0x02
	jr	NC, 00105$
;src/LevelLogic.c:65: global_object_information[e*3] = objects_map[e*3];
	ld	c, (hl)
	ld	b, #0x00
	ld	l, c
	ld	h, b
	...
	ld	l, e
	ld	(hl), a
;src/LevelLogic.c:66: global_object_information[e*3+1] = objects_map[e*3+1];
	ldhl	sp,	#5
	ld	a, (hl)
	ld	e, a
	add	a, a
	...
	ld	l, e
	ld	(hl), a
;src/LevelLogic.c:67: global_object_information[e*3+2] = objects_map[e*3+2];
	ldhl	sp,	#0
	ld	a, (hl)
	inc	a
	inc	a
...
;src/LevelLogic.c:69: }
	add	sp, #6
	ret
;src/LevelLogic.c:71: void read_global_block_info_from_map(unsigned char* blocks_map){
;	---------------------------------
...
_read_global_block_info_from_map::
	add	sp, #-1
	push	de
;src/LevelLogic.c:72: for(uint8_t e=0; e<NUMBER_OF_BLOCKS; e++){
	ldhl	sp,	#2
	ld	(hl), #0x00
00103$:
	ldhl	sp,	#2
	...
	sub	a, #0x04
	jr	NC, 00105$
;src/LevelLogic.c:73: global_blocks_available[e] = blocks_map[e];
...
;src/LevelLogic.c:75: }
	add	sp, #3
	ret
;src/LevelLogic.c:77: void change_colision_map_at(uint16_t tileindexBR, uint8_t new_value){
;	---------------------------------
...
_change_colision_map_at::
	...
	ldhl	sp,	#0
	ld	(hl), a
;src/LevelLogic.c:78: if(tileindexBR < 360){
	ld	c, e
	ld	b, d
	ld	a, c
	sub	a, #0x68
	...
	sbc	a, #0x01
	jr	NC, 00103$
;src/LevelLogic.c:79: global_colision_map[tileindexBR] = new_value;
	ld	hl, #_global_colision_map
	add	hl, de
	ld	e, l
	ld	d, h
	...
	ld	a, (hl)
	ld	(de), a
;src/LevelLogic.c:80: global_colision_map[tileindexBR-1] = new_value;
	ld	e, c
	ld	d, b
	dec	de
	ld	hl, #_global_colision_map
	...
	ld	a, (hl)
	ld	(de), a
;src/LevelLogic.c:81: global_colision_map[tileindexBR-20] = new_value;
	ld	a, c
	add	a, #0xec
	ld	e, a
	ld	a, b
	...
	ld	a, (hl)
	ld	(de), a
;src/LevelLogic.c:82: global_colision_map[tileindexBR-21] = new_value;
	ld	a, c
	add	a, #0xeb
	ld	c, a
	ld	a, b
...
;src/LevelLogic.c:84: }
	inc	sp
	ret
;src/LevelLogic.c:86: void change_colision_map_BR(uint16_t tileindexBR, uint8_t new_value){
;	---------------------------------
; Function change_colision_map_BR
; ---------------------------------
_change_colision_map_BR::
	ld	c, a
;src/LevelLogic.c:87: if(tileindexBR < 360){
	ld	a, e
	ld	l, d
	sub	a, #0x68
	ld	a, l
	sbc	a, #0x01
	ret	NC
;src/LevelLogic.c:88: global_colision_map[tileindexBR] = new_value;
	ld	hl, #_global_colision_map
	add	hl, de
	ld	(hl), c
;src/LevelLogic.c:90: }
	ret
;src/LevelLogic.c:92: uint8_t check_colision_of_sprites(uint8_t ax, uint8_t ay, uint8_t aw, uint8_t ah, uint8_t bx, uint8_t by, uint8_t bw, uint8_t bh){
;	---------------------------------
...
_check_colision_of_sprites::
	...
	ldhl	sp,	#5
	ld	(hl), e
;src/LevelLogic.c:93: uint8_t res = 0; //no colision of sprites
	ldhl	sp,	#0
	ld	(hl), #0x00
;src/LevelLogic.c:95: if(ax < bx + bw &&
	ldhl	sp,	#10
	ld	a, (hl)
	ldhl	sp,	#1
	ld	(hl+), a
...
;src/LevelLogic.c:98: ay + ah > by){
	...
	sbc	a, b
	jr	NC, 00102$
;src/LevelLogic.c:99: res = 1;
	dec	hl
	dec	hl
	ld	(hl), #0x01
00102$:
;src/LevelLogic.c:101: return res;
	ldhl	sp,	#0
	ld	a, (hl)
;src/LevelLogic.c:102: }
	add	sp, #6
	pop	hl
	add	sp, #6
	jp	(hl)
;src/LevelLogic.c:105: void get_init_point_from_map(uint8_t colision_map[360]){
;	---------------------------------
...
_get_init_point_from_map::
	dec	sp
	dec	sp
;src/LevelLogic.c:106: for(uint16_t i = 0; i<360; i++){
	xor	a, a
	ldhl	sp,	#0
	ld	(hl+), a
	ld	(hl), a
...
00105$:
	...
	sbc	a, #0x01
	jr	NC, 00107$
;src/LevelLogic.c:107: if(colision_map[i] == SOURCE){
	ld	l, c
	ld	h, b
	add	hl, de
	ld	a, (hl)
	sub	a, #0x03
	jr	NZ, 00106$
;src/LevelLogic.c:108: global_init_point = i;
	ldhl	sp,	#0
	ld	a, (hl)
...
;src/LevelLogic.c:112: }
	...
	inc	sp
	ret
;src/LevelLogic.c:114: void move_foward_block_id(void){
;	---------------------------------
; Function move_foward_block_id
; ---------------------------------
_move_foward_block_id::
;src/LevelLogic.c:115: global_selected_block++;
	ld	hl, #_global_selected_block
	inc	(hl)
;src/LevelLogic.c:116: if(global_selected_block >= 4){
	ld	a, (hl)
	sub	a, #0x04
	ret	C
;src/LevelLogic.c:117: global_selected_block = 0;
	ld	(hl), #0x00
