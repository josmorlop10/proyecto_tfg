...
;--------------------------------------------------------
	.area _CODE
;src/LevelLogic.c:22: void init_level(uint8_t level_number){
;	---------------------------------
; Function init_level
; ---------------------------------
_init_level::
	ld	c, a
;src/LevelLogic.c:24: global_keyset = 0;
	xor	a, a
	ld	(#_global_keyset),a
;src/LevelLogic.c:26: if(level_number >= sizeof(global_levels_array)){
;src/LevelLogic.c:27: level_number = level_number % sizeof(global_levels_array);
	ld	a,c
	cp	a,#0x02
	jr	C, 00115$
	and	a, #0x01
	ld	c, a
;src/LevelLogic.c:30: for(uint16_t i = 0; i<NUMBER_OF_TILES_IN_GRID; i++){
00115$:
	ld	de, #0x0000
00106$:
	ld	a, e
	...
	sbc	a, #0x01
	jr	NC, 00103$
;src/LevelLogic.c:31: global_colision_map[i] = EMPTY;
...
;src/LevelLogic.c:30: for(uint16_t i = 0; i<NUMBER_OF_TILES_IN_GRID; i++){
	...
	jr	00106$
00103$:
;src/LevelLogic.c:34: get_colision_from_map(global_levels_array[level_number], global_colision_map);
...
;src/LevelLogic.c:37: read_global_object_info_from_map(objects_map1_alt);
	ld	de, #_objects_map1_alt
	call	_read_global_object_info_from_map
;src/LevelLogic.c:38: read_global_block_info_from_map(blocks_map1_alt);
	ld	de, #_blocks_map1_alt
	call	_read_global_block_info_from_map
;src/LevelLogic.c:40: for(uint8_t e = 0; e < NUMBER_OF_BLOCKS; e++){
	ld	c, #0x00
00109$:
	ld	a, c
	sub	a, #0x06
	jr	NC, 00104$
;src/LevelLogic.c:41: update_values_in_hud(RIGHT+e, global_blocks_available[e]);
...
;src/LevelLogic.c:40: for(uint8_t e = 0; e < NUMBER_OF_BLOCKS; e++){
	...
	jr	00109$
00104$:
;src/LevelLogic.c:44: get_init_point_from_map(global_colision_map);
	ld	de, #_global_colision_map
;src/LevelLogic.c:46: }
	jp	_get_init_point_from_map
;src/LevelLogic.c:48: void update_game_state(GameState new_value){
;	---------------------------------
; Function update_game_state
; ---------------------------------
_update_game_state::
	ld	(#_global_game_state),a
;src/LevelLogic.c:49: global_game_state = new_value;
;src/LevelLogic.c:50: }
	ret
;src/LevelLogic.c:53: void get_colision_from_map(const unsigned char in[], uint8_t out[]){
;	---------------------------------
